...
class Instrument
  attr_accessor :ip, :session, :prev_mode, :port_list,
    ...
    :dl_ts , :datalog_flag, :hmid, :active_count
  CFG_LOC='/tmp/ORIG2'
  DL_PERIOD=60
    ...
    block_list.each { |block|
      ...
      if block_type == 1
        ...
        if (expected_test_count == test_count)
          ...
          $logger.debug "Running Test Trace Set"
          if (!DatalogProfile.test_trace_set(dl))
            $logger.debug "Failed to run test trace set."
          end
          if (!swp.test_trans_alarm(dl))
            $logger.debug "Failed to run test Noise Floor transistion"
          end
          test_count=0
        end
      else
  ...
  def measure_qam(freq, modulation_type, annex,symb_rate, polarity, cfgct_id)
    cfgct = CfgChannelTest.find(cfgct_id)
    if !(cfgct.preber_flag || cfgct.postber_flag)
      return nil
    end
    ...
    if (!digital_response.nil? && digital_response[:fwd_err_lock] == 1 && (cfgct.preber_flag || cfgct.postber_flag )) #If we Got Locks then lets try to get some really good measurements
      ...
      20.times do
        selected_socket_list=select([@command_io],nil,nil,1)
        if !(selected_socket_list.nil?)
          ret = qc.call(@command_io)
          if ret != Monitor::HEARTBEAT
            return false
          end
        end
      end
...
class Monitor
  ...
  def upgrade_firmware
    ...
    if !firmware_list.nil? && firmware_list.length ==1
      ...
      @instr_obj.session.upload_file(firmware.get_full_path(),'/usr/local/bin/at2000/at2500linux.run') {|pos,total| 
        update_status("Firmware transfer ",pos,total,instr_id)
        per=(pos.to_f/total.to_f*100.0).to_i
        instr.update_attributes(:firm_transfer=> per) if per > instr.read_attribute(:firm_transfer).to_i
      }
  ...
  def state_machine_iteration()
    if (@state == Analyzer::INGRESS)
      ...
      ch.cfg_channel_tests.each { |step|
        ...
        if modulation_reqs[:dcp] && step.test_requires(:dcp)#If this channel is a digital channel then lets just do the tests
          ...
          if modulation_reqs[:qam] && step.test_requires(:qam)   #Put flag check for qam measurments here
            ...
            if (measurements["dcp"].to_f < -15)
              ...
              return unless tmpmeas=$monitor_obj.instr_obj.measure_qam(ch.freq, ch.modulation, ch.annex,ch.symbol_rate, ch.polarity, step.id)
        ...
        $monitor_obj.instr_obj.store_measurements(measurements,step,site.id,@measure_cycle)
...
def command_driver(svr, tgt_cmd_port,state, timeout=1)
  puts svr.inspect()
