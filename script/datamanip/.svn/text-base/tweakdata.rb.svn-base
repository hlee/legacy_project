...
Site.find(:all).each { |site|
   limit=Limit.create(:measure_id=>meas.id, :channel_id=>ch.id, :site_id=> site.id,:min_val=>chfreq-.00001, :max_val=>chfreq+0.0001)
}
build_data(meas.id, 0.00001,chfreq*1, start_date, end_date,ch.id)
...
build_data(meas.id, 1.00001,audfreq*1, start_date, end_date,ch.id)
