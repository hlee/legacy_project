package com.sunrisetelecom.avantron
{
    ...
    public class Protocol extends Object
    {
        ...
        private static const INCR_DYN_RANGE:Number = 70 / 1024.0;
        ...
        private var m_trace_cycle:int=10;
        private var m_trace_count:int=0;

        private var wait_ack_count:int = 0;
        ...
        //////////////////////////
        public function Protocol(instr_address:String,instr_port:int,resp_callback:Function):void
        {
            m_socket.addEventListener(IOErrorEvent.IO_ERROR,ioErrorHandler);
            m_socket.addEventListener(SecurityErrorEvent.SECURITY_ERROR, securityHandler);
        ...
        {  
            ...
            switch(msg_type)
            {
                ...
                case 12341: //05 IMAGE TRACE
                    ...
                    callback("trace","response",{image:image,pct_image:pct_image});
                    //callback("trace","response",{image:image,pct_image:pct_image});
                    break;
        ...
        ////////////////////
        // qam_response
        // Parses the MSG_TYPE=07 messages from the avantron protocol.
        // packet - The raw packet data in the socket message
        ////////////////////
        private function qam_response(packet:ByteArray):void
        {
            ...
            packet.position=18;
            obj["polarity"]=packet.readUnsignedByte();
            obj["opt_att_value"]=packet.readUnsignedShort();
            obj["symbol_rate"]=packet.readUnsignedInt();
            obj["ber_pre"]=packet.readFloat();
            obj["ber_post"]=packet.readFloat();
            obj["mer"]=packet.readUnsignedShort();
            obj["enm"]=packet.readUnsignedShort();
            obj["evm"]=packet.readUnsignedShort();
            obj["es"]=packet.readUnsignedInt();
            obj["ses"]=packet.readUnsignedInt();
            obj["fls"]=packet.readUnsignedInt();
            obj["unavail"]=packet.readUnsignedInt();
            obj["symb_lock"]=packet.readByte();
            obj["fwd_err_lock"]=packet.readByte();
            obj["stream_lock"]=packet.readByte();
            obj["resv"]=packet.readByte();
            obj["const_points"]=[];
            var const_points:Number=packet.readUnsignedShort();
            var datapoint:Array=[];
            //trace ("Const Points:"+const_points.toString());
            //trace ("ber post:"+obj["ber_post"].toString());
            //trace ("fwd err lock:"+obj["fwd_err_lock"].toString());
            //trace ("stream_lock:"+obj["stream_lock"].toString());
            for (var i:int=0;i<const_points;i++)
            {
                datapoint=[];
                datapoint[0]=packet.readByte();
                datapoint[1]=packet.readByte();
                obj["const_points"][i]=datapoint;
                //trace("PT:"+obj["const_points"][i]);
            }
            callback("constellation","response",obj);
            //callback("constellation","response",obj);
            //So we should have a Constellation.
        }
        ...
        private function general_response(packet:ByteArray):void
        {
            ...
            switch(packet[17])
            {
                ...
                case 17:
                    ...
                    //When I review the code, find there is no break. --Jerry
                    break;
                case 20:
        ...
        public function setSettings(mode:int,settings:SettingsObj):void
        {
        ...
        public function onSocketData(event:ProgressEvent):void
        {
            ...
            {
                trace(m_socket.bytesAvailable);
                bytes=readPacket();
                //If we have a large queue and the current packet is a trace. Then lets skip it.
           
                if ((m_socket.bytesAvailable < 1000) || (bytes[7]*256+bytes[6] != 12341))
                {
                    parsePacketData(bytes); 
                }
                else
                {
                    trace("Skipping data because bytes available = " + m_socket.bytesAvailable.toString());
                }  
            }
        }
        ...
        private function readPacket():ByteArray
        {
            ...
            if (packetPrefixFound)
            {
                ...
                while(readMore)
                {
                    ...
                    if (try_count >= 15)
                    {
                        trace("Abandon record,LEN:" + length + ", bytes avail:" + m_socket.bytesAvailable);
